...
import { MaxRefcLat } from './components/MaxRefcLat';
import { MinRefcLat } from './components/MinRefcLat';
...
// 'fetch' ed from ./components/References.js using the useEffect Hook.
  ...
  const [maxrefclat,setMaxRefCLat] = useState([]);
  const [maxrefclatval,setMaxRefCLatval] = useState(30);
  const [maxrefclatvalErr,setMaxRefCLatvalErr] = useState({});

  const [minrefclat,setMinRefCLat] = useState([]);
  const [minrefclatval,setMinRefCLatval] = useState(-30);
  const [minrefclatvalErr,setMinRefCLatvalErr] = useState({});
  ...
  useEffect ( () => {
      fetch("/serveViewZenithAngleChoices").then(
          res => res.json()
      .then(
          data =>   {
              setMaxVZA(data)
  ...
  const handleSubmit = (event) => {event.preventDefault()
      ...
      var jsonData = [
          ...
          {"key": "Targets:Target:Constraints:Constraint:ConstraintName:max_viewing_zenith_angle", "value": vzaval},
