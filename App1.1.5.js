...
import { CheckSciencePlansButton } from "./components/CheckSciencePlansButton";
import { CheckSciencePlanIDButton } from "./components/GetSciencePlanIDButton";
import { CheckSciencePlanStatusButton } from "./components/GetSciencePlanStatusButton";
...
function App() {
  ...
  const [textvalErr, setTextvalErr] = useState({});
  
  const [sciencePlanID, setSciencePlanID] = useState(0);
  const [userPlanIDval, setUserPlanIDval] = useState({});
  ...
  const updateSciencePlanID = async () => {
    fetch("/serveSciencePlanID").then((res) =>
      res.json().then((data) => {
        ...
        console.log('SPS Plan ID returned is: ', data);
      }) )
   }

  const updateSciencePlanStatus = async () => {
    var userInputPlanIDData = [ {key: "PlanIDInput", value: userPlanIDval} ];
    setUserPlanIDval();
    //await fetch("/serveSciencePlanStatus", {
    await fetch("/serveSciencePlanStatus", {
        method: "POST", // or 'PUT'
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(userInputPlanIDData),
      }).then(response => {
         console.log('SPS Plan Status returned is: ', response); 
         //setSciencePlanStatus(response.text());
  ...
  const retrieveSciencePlan = async () => {
    fetch("/retrieveSciencePlan").then((res) =>
      res.json().then((data) => {
        setSciencePlanStatus(data);
        console.log(data);
      }) )
  ...
  const fetchSciencePlanandPlot = async () => {
    // get the data from the api
      const ScienceplanandplotData = await fetch("/serveSciencePlanandPlot");
    // convert the planandplotData to json
      const planandplotData = await ScienceplanandplotData.json();
    // set state with the result if `isSubscribed` is true
      setPlanandPlotData(planandplotData);
  }
  ...
  const handleSubmit = (event) => {
    ...
    if (isValid) {
      ...
      //}).then(response => {
        //console.log("Second Then block ",response); 
        //updateSciencePlanStatus();
         return fetchSciencePlanandPlot();
      })
    // closing brace for isValid
    }
  ...
  return (
    <div className='App'>
      <h2> LASICS (Beta) </h2>
        ...
        {/* this component renders the 3-buttons above the Target portion */}
        <h4> SciencePlans </h4>
        <CheckSciencePlansButton />
        <br />
      ...
      <form>
        <br />
        {/* A JSX comment */}
        {/* <TextField fieldLabel={"Please Enter Science Plan ID"} fieldType={"PlanID"} onTextFieldChange={()=>{setUserPlanIDval()}} /> */}
        <TextField fieldLabel={"Please Enter Science Plan ID"} fieldType={"PlanID"} onTextFieldChange={setUserPlanIDval} />
        <br />
        <button id='planStatusButton' type='button' onClick={getSciencePlanStatus}> LASICS SCIENCE PLAN STATUS </button> 
      </form>
