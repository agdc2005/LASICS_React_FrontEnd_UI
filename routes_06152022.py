...
app.secret_key = 'any rand0m 5tr1ng'
...
# This get_TargetAltitudes.py file is in directory immediately above the directory containing routes.py
from get_TargetAltitudes import get_TargetAltitudes
...
# -->>> For Target 1 = LEO-LASICS AND Target 2 = LEO-LASICS
...
# -->>> For Target 1 = LEO-LASICS AND (Target 2 == LandNadir-LASICS TargetType OR Target 2 == GEO-LASICS TargetType)
...
# --<<<
    ...
    #
    print(' ')
    print('******************* Begin User Request JSON Object ****************************************')
    print(' ')
    ...
    print('******************* End User Request JSON Object ****************************************')

    # Get target2Type for both Reference_Name and Target_Name
    reference2Type = get_target2Type(Reference_Name)
    target2Type    = get_target2Type(Target_Name)
    #print('Target 1 Type is: ', reference2Type)
    ...
    if ( reference2Type != 'LandNadir-LASICS' and target2Type != 'LandNadir-LASICS'):
      Reference_Altitude = ' '
      Target_Altitude    = ' '
      Reference_Altitude, Target_Altitude = get_TargetAltitudes(Reference_Name,Target_Name)
      Target_Swap_Name = Target_Name      # Target_Name (Target 2) as originally selected on React front-end UI
      if int(Target_Altitude) < int(Reference_Altitude):
        Target_Name = Reference_Name
        Reference_Name = Target_Swap_Name 
        print('Target 1 changed to: ', Reference_Name,':',Target_Altitude)
        print('Target 2 changed to: ', Target_Name,':',Reference_Altitude)
      print('--------------------------------------------------------------')
    if ( reference2Type == 'LandNadir-LASICS' and target2Type != 'LandNadir-LASICS'):
      Target_Swap_Name = Target_Name      # Target_Name (Target 2) as originally selected on React front-end UI
      Target_Name = Reference_Name
      Reference_Name = Target_Swap_Name 
      print('Target 1 changed to: ', Reference_Name)
      print('Target 2 changed to: ', Target_Name)
      print('--------------------------------------------------------------')
    #
    # Create Unique User ID
    user_UUID = uuid.uuid4()
    # Create an ASCII Text File for email to User that shows their UI Selections
    print('/echoJSON route User Unique ID: ', user_UUID)
    #sys.exit()
    #
...
# call the create_SPS_XML_Request Module and  update the XML document
    ...
    # This file is in directory above current working directory with routes.py file
    ...
    print('Target1_Name: ', Reference_Name)
    print('Target2_Name: ', Target_Name)
    xmlFile = get_xmlFileTemplate(Reference_Name,Target_Name)
    ...
    print('********************RESPONSE RECEIVED from LASICS SPS***************************')
    #print('/echoJSON route: ', SPS_Server_Response)
    ...
    print('/echoJSON route for user_UUID: ', user_UUID, ' plan_ID is: ', str(plan_ID))
    session['user_UUID'] = user_UUID
    session['plan_ID'] = str(plan_ID)
    session['Reference_Name'] = Reference_Name
    session['Target_Name'] = Target_Name
    session['PlanStartEpochDate'] = PlanStartEpochDate
    session['User_Email'] = Requester_Email
    ...
    session['plan_Status'] = str(plan_Status)
    print('********************RETURN from echoJSON route***************************')
    #return()
...
import os
@app.route('/serveSciencePlanandPlot', methods=['GET'])
def serveSciencePlanandPlot():
    # 
    plan_ID = session.get('plan_ID',None)
    user_UUID = session.get('user_UUID',None)
    plan_Status = session.get('plan_Status',None)
    Reference_Name = session.get('Reference_Name',None)
    Target_Name = session.get('Target_Name',None)
    PlanStartEpochDate = session.get('PlanStartEpochDate', None)
    ...
    # get planStatus Response as below
    SPS_Server_Response = send_PlanStatus_Request_to_SPS_Server(SPS_PlanStatus_Request)
    print('/serveSciencePlanandPlot route: ', SPS_Server_Response)
    # extract/get Initial planStatus value
    plan_Status = get_planStatus_from_SPS_Server_Response(SPS_Server_Response)
    print('/serveSciencePlanandPlot route For user_UUID: ', user_UUID, ' plan_Status: ', plan_Status, 'User_Email: ', User_Email)
    startTime = time.time()
    ...
    if ( plan_Status == 0 ):
      ...
      while not FoundPlan:
           ...
           if ( plan_Status == 1):  #No errors and plan completed successfully
             ...
             # Plan may contain ZERO Science Opportunities
             ...
             SciencePlanFilePath = './tmp/'+str(user_UUID)+'/'
             if not os.path.exists(SciencePlanFilePath):
                os.makedirs(SciencePlanFilePath)
             SciencePlanFileName = SciencePlanFilePath+'LASICS_SciencePlan'+'.xml'
             #with open("LASICS_SciencePlan_"+now_date.isoformat()+".xml",'w') as fout:
             with open(SciencePlanFileName,'w') as fout:
                fout.write(SPS_Server_Response)
             fout.close()
             print('--------------LASICS Science Plan XML File Created Status = 1 -------------------------------------')
             #Parse PlanResultsFile to see if there are more than ZERO Science Opportunities
             # If more than 0 Science Opportunities, the get the plan plot by sending planPlot Request
             SPS_PlanPlot_Request = '<SPS_Request><Request>getPlanPlot</Request><planID>'+str(plan_ID)+'</planID></SPS_Request>'
             SPS_Server_Response = send_PlanPlot_Request_to_SPS_Server(SPS_PlanPlot_Request,user_UUID)
             PlanPlotFileName = SPS_Server_Response
             #   
             # email User results to User when available
             # email is : User_Request_Data['Requester_Email']
             #return(json.dumps(SciencePlanFileName))
             FoundPlan = True
             break
           ...
           if ( plan_Status == 3):
             ...
             if not os.path.exists(SciencePlanFilePath):
                os.makedirs(SciencePlanFilePath)
             SciencePlanFileName = SciencePlanFilePath+'LASICS_SciencePlan'+'.xml'
             #SciencePlanFileName = "LASICS_SciencePlan_"+now_date.isoformat()+".xml"
             with open(SciencePlanFileName,'w') as fout:
             #with open("LASICS_SciencePlan_"+now_date.isoformat()+".xml",'w') as fout:
             ...
             fout.close()
             print('--------------LASICS Science Plan Created w Error Status = 3 -------------------------------------')
             # email User results to User when available
           ...
           if ( plan_Status == 4):
             ...
             #with open("LASICS_SciencePlan_"+now_date.isoformat()+".xml",'w') as fout:
                fout.write(SPS_Server_Response)
             fout.close()
             print('--------------LASICS Science Plan Created w Error Status = 4-------------------------------------')
             # Critical SPS Error
             # email User results to User when available
             # email is : User_Reqeust_Data['Requester_Email']
             return(json.dumps(SciencePlanFileName))
    ...
    return { 'User_Email': User_Email,
             ...
             'PlanPlotFile': PlanPlotFileName,
             'SciencePlanFile': SciencePlanFileName
    }
    #return redirect(url_for("userInput", refname=Reference_Name))
#
@app.route('/serveSciencePlanID', methods=['GET'])
def serveSciencePlanID():
   # session['user_UUID'] = user_UUID
   # session['plan_ID'] = str(plan_ID)
   #if 'plan_ID' in session:
    plan_ID = session.get('plan_ID',None)
    return(json.dumps(str(plan_ID)))

#This send_PlanStatus_Request_to_SPS_Server.py file is in directory immediately above this routes.py file
from send_PlanStatus_Request_to_SPS_Server import send_PlanStatus_Request_to_SPS_Server
# This get_planStatus_from_SPS_Server_Response.py file is in directory immediately above this routes.py file 
from get_planStatus_from_SPS_Server_Response import get_planStatus_from_SPS_Server_Response
@app.route('/serveSciencePlanStatus', methods=['GET'])
def serveSciencePlanStatus():
   # session['user_UUID'] = user_UUID
   # session['plan_ID'] = str(plan_ID)
   #if 'plan_ID' in session:
    plan_Status = session.get('plan_Status',None)
    ...
    SPS_Server_Response = send_PlanStatus_Request_to_SPS_Server(SPS_PlanStatus_Request)
    plan_Status = get_planStatus_from_SPS_Server_Response(SPS_Server_Response)
    return(json.dumps(str(plan_Status)))
